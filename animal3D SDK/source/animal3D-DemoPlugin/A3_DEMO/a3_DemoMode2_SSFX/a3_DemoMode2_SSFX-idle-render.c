...
void a3ssfx_render(a3_DemoState const* demoState, a3_DemoMode2_SSFX const* demoMode, a3f64 const dt)
{
	...
	if (renderMode == ssfx_renderModePhongDL)
	{
		...
		a3framebufferBindColorTexture(demoState->fbo_c16x4_d24s8, a3tex_unit04, 1); //nrm
		a3framebufferBindDepthTexture(demoState->fbo_c16x4_d24s8, a3tex_unit05);
		...
		// from https://www.learnopengles.com/tag/additive-blending/
		glEnable(GL_BLEND);
	...
	case ssfx_renderModePhongDL:
		...
		a3framebufferBindColorTexture(demoState->fbo_c16x4, a3tex_unit05, 0); // diffuse light
		a3framebufferBindColorTexture(demoState->fbo_c16x4, a3tex_unit06, 1); // specular light
		//...
		break;
	}
